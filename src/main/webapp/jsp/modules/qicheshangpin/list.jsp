...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">汽车商品管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>汽车商品列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">汽车商品列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<select name="qichepinpai" id="qichepinpaiSelect" class="form-control form-control-sm" aria-controls="tableId">
									</select>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('zixundianhua')">咨询电话<i id="zixundianhuaIcon" class="fa fa-sort"></i></th>
											<th>宣传视频</th>
											<th onclick="sort('shangshiriqi')">上市日期<i id="shangshiriqiIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			var qichepinpaiIndex = document.getElementById("qichepinpaiSelect").selectedIndex;
			var qichepinpaiOptions = document.getElementById("qichepinpaiSelect").options;
			var qichepinpaiSelectedOption = qichepinpaiOptions[qichepinpaiIndex]
			if(qichepinpaiSelectedOption.text != null && qichepinpaiSelectedOption.text != ''){
				searchForm.qichepinpai = qichepinpaiSelectedOption.text;
			}
			getDataList();
		}		
		// 获取数据列表
        function getDataList() {
			http("qicheshangpin/page","GET",{
				...
				qichepinpai : searchForm.qichepinpai,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var zixundianhuaCell  = document.createElement('td');

						 zixundianhuaCell.innerHTML = item.zixundianhua;

			row.appendChild(zixundianhuaCell);
			var xuanchuanshipinCell  = document.createElement('td');

						  var xuanchuanshipinMedia = document.createElement('button');
			  xuanchuanshipinMedia.setAttribute('class','btn-table')
			  xuanchuanshipinMedia.innerHTML = '播放';
			  var xuanchuanshipinAttr = "mediaPlay('" + baseUrl+ item.xuanchuanshipin + "')"
			  xuanchuanshipinMedia.setAttribute('onclick',xuanchuanshipinAttr);
       		          xuanchuanshipinCell.appendChild(xuanchuanshipinMedia);


			row.appendChild(xuanchuanshipinCell);
			var peizhijieshaoCell  = document.createElement('td');

			
			var shangshiriqiCell  = document.createElement('td');

						  shangshiriqiCell.innerHTML = item.shangshiriqi;


			row.appendChild(shangshiriqiCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "预约试驾"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'yuyuejilu','','','')");
			if(crossBtnControl('预约试驾') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("qicheshangpin/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discussqicheshangpin/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"qicheshangpin");
